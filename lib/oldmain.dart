  ...
  Future<void> _initializeSerialService() async {
    try {
      ...
      if (initializationSuccessful) {
        ...
        // Set up the message subscription to send messages to the serial output
        _messageSubscription = webSocketService.messages.listen(
          (message) {
            setState(() {
              _lastMessage = message;
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: _isSerialInitialized
                          ? null
                          : () async {
                              await _initializeSerialService();
                              if (_isSerialInitialized) {
                                // Start sending serial data
                                // serialService.sendSerial(_lastMessage);
                              }
                            },
                      child: Text(_isSerialInitialized
                          ? 'Serial Initialized'
                          : 'Initialize Serial'),
                    ),
                  ],
